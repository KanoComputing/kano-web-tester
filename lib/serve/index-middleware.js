...
module.exports = (opts = {}) => {
    ...
    if (webTesterExample) {
        data.FAVICON = '../../assets/favicon.ico';
    } else {
        const icoPath = resolve.sync('@kano/web-tester/assets/favicon.ico', { basedir: cwd });
        data.FAVICON = `/${path.relative(cwd, icoPath).replace(/\\/g, '/')}`;
    }
